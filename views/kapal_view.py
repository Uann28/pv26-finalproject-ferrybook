...
class KapalDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.kap_p_input = QSpinBox(); self.kap_p_input.setRange(1, 9999); self.kap_p_input.setSuffix(" orang"); self.kap_p_input.setFixedHeight(36)
        self.kap_k_input = QSpinBox(); self.kap_k_input.setRange(1, 999); self.kap_k_input.setSuffix(" unit"); self.kap_k_input.setFixedHeight(36)
        ...
        form.addRow("Kapasitas Penumpang *", self.kap_p_input)
        form.addRow("Kapasitas Kendaraan *", self.kap_k_input)
    ...
    def _save(self):
        ...
        if self.kapal:
            ...
            ok, msg = models.create_kapal(
                nama, kode, self.kap_p_input.value(),
                self.kap_k_input.value(), self.ket_input.toPlainText()
            )
        if ok:
            self.accept()
        else:
...
class KelolaDKapalView(QWidget):
    COLS = ["Nama Kapal", "Kode", "Kap. Penumpang", "Kap. Kendaraan", "Status", "Aksi"]
    ...
    def _build_ui(self):
        ...
        hh.setSectionResizeMode(5, QHeaderView.Fixed);   self.table.setColumnWidth(5, 180)
    ...
    def _refresh(self):
        ...
        for i, k in enumerate(rows):
            ...
            kp_item = QTableWidgetItem(f"{k['kapasitas_penumpang']} orang"); kp_item.setTextAlignment(Qt.AlignCenter)
            kk_item = QTableWidgetItem(f"{k['kapasitas_kendaraan']} unit"); kk_item.setTextAlignment(Qt.AlignCenter)
            self.table.setItem(i, 0, nama_item)
            self.table.setItem(i, 1, kode_item)
            self.table.setItem(i, 2, kp_item)
            self.table.setItem(i, 3, kk_item)

            self.table.setCellWidget(i, 4, _status_cell(bool(k['is_active'])))
            self.table.setCellWidget(i, 5, _action_cell(
