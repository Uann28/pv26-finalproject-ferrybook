...
class JadwalDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.jam_tiba_input = QTimeEdit()
        self.jam_tiba_input.setTime(QTime(9, 30))
        self.jam_tiba_input.setFixedHeight(36)
        self.jam_tiba_input.setDisplayFormat("HH:mm")
        ...
        g2_layout.addWidget(QLabel("Jam Tiba (Est.)"), 2, 0); g2_layout.addWidget(self.jam_tiba_input,  2, 1)
        layout.addWidget(g2)

        g3 = QGroupBox("Kapasitas")
        g3_layout = QGridLayout(g3); g3_layout.setSpacing(10)
        self.kap_p_input = QSpinBox(); self.kap_p_input.setRange(1, 9999); self.kap_p_input.setValue(300); self.kap_p_input.setFixedHeight(36)
        self.kap_k_input = QSpinBox(); self.kap_k_input.setRange(1, 999);  self.kap_k_input.setValue(50);  self.kap_k_input.setFixedHeight(36)
        g3_layout.addWidget(QLabel("Kapasitas Penumpang *"), 0, 0); g3_layout.addWidget(self.kap_p_input, 0, 1)
        g3_layout.addWidget(QLabel("Kapasitas Kendaraan *"), 1, 0); g3_layout.addWidget(self.kap_k_input, 1, 1)
        layout.addWidget(g3)
    ...
    def _load_combos(self):
        ...
        if self.kapals and not self.jadwal:
            k = self.kapals[0]
            self.kap_p_input.setValue(k['kapasitas_penumpang'])
            self.kap_k_input.setValue(k['kapasitas_kendaraan'])
    ...
    def _populate(self, j):
        ...
        if j.get('jam_tiba'):
            self.jam_tiba_input.setTime(QTime.fromString(j['jam_tiba'], "HH:mm"))
        self.kap_p_input.setValue(j['kapasitas_penumpang'])
        self.kap_k_input.setValue(j['kapasitas_kendaraan'])
    ...
    def _save(self):
        ...
        jam_tiba  = self.jam_tiba_input.time().toString("HH:mm")
        if self.jadwal:
            ok, msg = models.update_jadwal(
                ...
                self.kap_p_input.value(), self.kap_k_input.value()
            )
        else:
...
class KelolaJadwalView(QWidget):
    ...
    def _build_ui(self):
        ...
        hh.setSectionResizeMode(5, QHeaderView.Fixed); self.table.setColumnWidth(5, 108)
        hh.setSectionResizeMode(6, QHeaderView.Fixed); self.table.setColumnWidth(6, 108)
        hh.setSectionResizeMode(7, QHeaderView.Fixed); self.table.setColumnWidth(7, 180)
    ...
    def _populate_table(self):
        ...
        for i, j in enumerate(rows):
            ...
            for c, v in enumerate([
                ...
                j.get('jam_berangkat', ''),
                j.get('jam_tiba', ''),
                f"{j['terisi_penumpang']} / {j['kapasitas_penumpang']}",
                f"{j['terisi_kendaraan']} / {j['kapasitas_kendaraan']}",
            ], start=1):
                item = QTableWidgetItem(v)
                item.setTextAlignment(Qt.AlignCenter)
                self.table.setItem(i, c, item)

            self.table.setCellWidget(i, 7, _action_cell(
